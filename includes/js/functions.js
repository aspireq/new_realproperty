...
//        }
    ...
    $("#btn_step6").click(function (e) {
        ...
        if ($('.flat_amenities_check:checked').length > 0) {
            ...
            call_error_step6("flat_amenities_error", "Please select flat amenities.");
            submit_false = 1;
        }
        if ($('.project_amenities_check:checked').length > 0) {
            submit_false = (submit_false == 1) ? 1 : 0;
        } else {
            call_error_step6("project_amenities_error", "Please select project amenities.");
            submit_false = 1;
        }
        if ($('#valid_image_error').val() == 1) {
            submit_false = (submit_false == 1) ? 1 : 0;
        } else {
